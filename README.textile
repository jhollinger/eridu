...
<pre>
git clone git://github.com/jhollinger/eridu.git enki
cd eridu
bundle install              # Install all the required gems

cp config/database.yml.template config/database.yml
cp config/eridu.yml.template config/eridu.yml
# Edit the above configuration files to taste
# Optionally, also copy and edit mail.yml.template and recaptcha.yml.template
...
* Atom is a good publishing standard
...
* Everyone wants live comment preview, reCAPTCHA spam protection, and email notifications
...
Framework choices aside, Eridu differs from its progenitor, Enki, mainly in the number of bundled features. These include reCAPTCHA spam protection, email notifications, and a responsive layout. While Enki leaves these "extras" as an exercise to the reader, Eridu pragmatically includes these de facto necessities.
...
Enki and Eridu are so similar that @rake db:migrate_from_enki@ easily converts an Enki database to Eridu (it will leave some unused tables and columns behind, though). Also their URL structures are identical, so your existing links and page rankings won't even notice.
